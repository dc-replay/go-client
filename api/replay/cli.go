...
func (c *Cli) OpenApp(id uint) error {
	err := c.HttpJsonGet(fmt.Sprintf("/api/v1/app/run/%d", id), nil)
	return err
}

func (c *Cli) ConfigGet(k string) (ret string, err error) {
	err = c.HttpJsonGet("/api/v1/config/byrepo/"+util.Repo()+"/"+k, &ret)
	return
}

func (c *Cli) ConfigGetAll() (ret []types.Config, err error) {
	ret = make([]types.Config, 0)
	err = c.HttpJsonGet("/api/v1/config/byrepo/"+util.Repo(), &ret)
	return
}

func (c *Cli) CronReload() error {
	return c.HttpJsonGet("/ipc/cron/reload", nil)
}

func (c *Cli) Exit() error {
	err := c.HttpJsonGet("/api/v1/exit", nil)
	return err
}

func (c *Cli) MenuGetAllEnabled() (ret []types.Menu, err error) {
	ret = make([]types.Menu, 0)
	err = c.HttpJsonGet("/api/v1/menu/enabled", nil)
	return ret, err
}

func (c *Cli) QueueAdd(job string, bs []byte) error {
	return c.HttpJsonGet("/api/v1/queue/add/"+job, bs)
}
func (c *Cli) QueueGetData(id string) (ret []byte, err error) {
	ret, err = c.HttpRawGet("/api/v1/queue/getrequest/" + id)
	return
}
...
func (c *Cli) QueueAbort() error {
	return c.HttpJsonGet("/api/v1/queue/abort", nil)
}
func (c *Cli) QueueEnqueue(id uint) error {
	return c.HttpJsonGet(fmt.Sprintf("/api/v1/robots/op/enqueue/%d", id), nil)
}

func (c *Cli) ServiceStopAll() error {
	return c.HttpJsonGet("/api/v1/service/op/stopall", nil)
}
...
func (c *Cli) SQL(s string) (*SQLReturn, error) {
	...
	out := &SQLReturn{}
	err := c.HttpJsonPost("/api/v1/sql", in, &out)
	return out, err
}
